/**
 * @import {Construct, Effects, State, Token, TokenizeContext, Tokenizer} from 'micromark-util-types'
 ...
 * @returns {Construct}
 */
export function directiveContainer(options) {
  /** @type {Construct} */
  const attributes = {
  ...
  return {
    tokenize(effects, ok, nok) {
      return tokenizeDirectiveContainer.call(this, effects, ok, nok, attributes)
    },
    concrete: true,
  }
}
 ...
 * @returns {State}
 */
function tokenizeDirectiveContainer(effects, ok, nok, attributes) {
  const self = this
